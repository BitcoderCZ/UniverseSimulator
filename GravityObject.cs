...
namespace UniverseSimulator
{
    public class GravityObject
    {
        ...
        protected List<Vector2I> trailPoints;
        protected UIText infoText;
        public uint infoTextId;
        protected int index;

        public GravityObject(Vector2F _pos, float _mass, float _radius, Vector2F _initialVelocity, UIManager ui, int _index)
        {
            Pos = _pos;
            lastX = (int)Pos.X;
            lastY = (int)Pos.Y;
            ...
            trailPoints = new List<Vector2I>();
            infoText = new UIText("Planet", (int)Pos.X, (int)Pos.Y, Universe.CLR_GREEN, 16);
            infoTextId = ui.AddElement(infoText);
            index = _index;
        }

        public void UpdateVelocity(List<GravityObject> allGOs, float delta)
        {
            for (int i = 0; i < allGOs.Count; i++)
                if (allGOs[i] != this) {
                    float distSqrt = (allGOs[i].Pos - Pos).SqrtLength();
                    if (distSqrt == 0f)
                        continue;
                    Vector2F forceDir = (allGOs[i].Pos - Pos).Normalized();
                    Vector2F acceleration = forceDir * Universe.Gravity * allGOs[i].Mass / distSqrt;
        ...
        public virtual void Draw()
        {
            if ((int)Pos.X >= 0 && (int)Pos.Y >= Game.barHeight && (int)Pos.X < (int)Program.game.trails.Width && Pos.Y < Program.game.trails.Height) {
                ...
                Program.game.FillCircle((Vector2I)Pos, (int)Radius, Color.White);
